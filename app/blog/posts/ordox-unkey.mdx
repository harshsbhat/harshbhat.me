...
Now that the app was in production, there was a chance someone could write a bash script called my API, which essentially made me run out of all the OpenAI tokens.
...
Setting rate limits with Unkey is a truly enjoyable experience, and I’m not even exaggerating.Setting rate limits with Unkey is a truly enjoyable experience, and I’m not even exaggerating.
...
So while initializing unkey we had the limit set to `2` right? Let's quicly change it to `8`. While using this rate limit let's pass one more vlaue called cost kind of like this.
